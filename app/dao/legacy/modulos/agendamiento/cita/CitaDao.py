...
class CitaDao:
    ...
    def guardarCita(self, id_paciente, id_agenda_horario, id_especialista, id_especialidad,
        ...
        try:
            ...
            try:
                # Obtener el nombre del estado inicial (usualmente "AGENDADA")
                cur.execute("SELECT est_cita_nombre FROM estados_citas WHERE id_estado_cita = %s", (id_estado_cita,))
                estado_row = cur.fetchone()
                nombre_estado_inicial = estado_row[0] if estado_row else 'AGENDADA'
                
                log_dao = CitaLogDao()
                log_dao.registrar_cambio(
                    id_cita=cita_id,
                    estado_nuevo=nombre_estado_inicial,
                    estado_anterior=None,
                    motivo_cambio="Creación de cita",
                    usuario=cita_creacion_usuario
                )
            except Exception as e:
                app.logger.warning(f"Error al registrar log inicial para cita {cita_id}: {str(e)}")
    ...
    def cambiarEstadoCita(self, id_cita, id_estado_cita, usuario_id=1):
        ...
        try:
            ...
            cur.execute("SELECT est_cita_nombre FROM estados_citas WHERE id_estado_cita = %s", (id_estado_cita,))
            estado_nuevo_row = cur.fetchone()
            estado_nuevo = estado_nuevo_row[0] if estado_nuevo_row else str(id_estado_cita)
            ...
            try:
                ...
                app.logger.warning(f"Error al registrar log de cambio de estado para cita {id_cita}: {str(e)}")
    ...
    def getCitasHoyByEspecialista(self, id_especialista, fecha=None):
        """
        Obtiene las citas del día para un especialista (Mi Agenda).
        Si no se pasa fecha, usa la fecha de hoy.
        Retorna campos completos incluyendo id_paciente y cita_motivo.
        """
        ...
        citasSQL = """
            ...
            ORDER BY c.cita_hora_inicio ASC
        """
        ...
        try:
            cur.execute(citasSQL, (id_especialista, fecha))
            citas = cur.fetchall()

            return [{
                'id_cita': c[0],
                'id_paciente': c[1],
                'cita_fecha': c[2].strftime('%d/%m/%Y') if c[2] else None,
                'cita_hora_inicio': c[3].strftime('%H:%M') if c[3] else None,
                'cita_hora_fin': c[4].strftime('%H:%M') if c[4] else None,
                'cita_tipo': 'PRIMERA_VEZ' if c[5] else 'SEGUIMIENTO',
                'paciente_nombre': c[6],
                'historia_clinica': c[7],
                'paciente_telefono': c[8],
                'cita_motivo': c[9],
                'numero_sesion': c[10],
                'especialidad': c[11],
                'estado_nombre': c[12],
                'estado_color': c[13] if c[13] else '#6c757d',
                'id_estado_cita': c[14]
            } for c in citas]

        except Exception as e:
            app.logger.error(f"Error en getCitasHoyByEspecialista: {str(e)}")
            return []
        finally:
            cur.close()
            con.close()
    ...
    def cambiarEstadoCita(self, id_cita, nombre_estado):
        """
        Cambia el estado de una cita buscando el estado por nombre.
        Retorna True si tuvo éxito, False si falló.
        Usado por la Ventana de Consulta Unificada (Fase 2).
        """
        conexion = Conexion()
        con = conexion.getConexion()
        cur = con.cursor()
        ...
        try:
            ...
            cur.execute(
                "SELECT id_estado_cita FROM estados_citas WHERE est_cita_nombre = %s AND est_cita_activo = TRUE",
                (nombre_estado,)
            )
            row = cur.fetchone()
            if not row:
                app.logger.error(f"cambiarEstadoCita: Estado '{nombre_estado}' no encontrado en estados_citas")
                return False

            id_estado = row[0]

            cur.execute(
                "UPDATE citas SET id_estado_cita = %s WHERE id_cita = %s AND cita_activo = TRUE",
                (id_estado, id_cita)
            )
            con.commit()
            app.logger.info(f"cambiarEstadoCita: Cita {id_cita} cambiada a '{nombre_estado}' (id={id_estado})")
            
            # ✅ Registrar en log
            try:
                if estado_anterior != nombre_estado:
                    log_dao = CitaLogDao()
                    usuario = session.get('usuario_username', 'SISTEMA')
                    log_dao.registrar_cambio(
                        id_cita=id_cita,
                        estado_nuevo=nombre_estado,
                        estado_anterior=estado_anterior,
                        motivo_cambio="Cambio de estado desde Consulta Unificada",
                        usuario=usuario
                    )
            except Exception as e:
                app.logger.warning(f"Error al registrar log de cambio de estado unificado para cita {id_cita}: {str(e)}")

            return True

        except Exception as e:
            app.logger.error(f"Error en cambiarEstadoCita: {str(e)}")
            con.rollback()
            return False
        finally:
            cur.close()
