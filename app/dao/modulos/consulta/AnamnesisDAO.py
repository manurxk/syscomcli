...
class AnamnesisDao:
    ...
    def getAnamnesisByPaciente(self, pac_id):
        """Obtiene la anamnesis activa de un paciente con datos completos"""
        anamnesisSQL = """
            SELECT
                a.id_anamnesis,
                a.id_paciente,
                ...
                pf1.per_nombre || ' ' || pf1.per_apellido AS elaborado_por_nombre,
                pf2.per_nombre || ' ' || pf2.per_apellido AS modificado_por_nombre
            FROM anamnesis a
            JOIN pacientes pac ON a.id_paciente = pac.id_paciente
            JOIN personas p ON pac.id_persona = p.id_persona
            LEFT JOIN usuarios u1 ON a.elaborado_por = u1.id_usuario
            LEFT JOIN funcionarios f1 ON u1.id_funcionario = f1.id_funcionario
            LEFT JOIN personas pf1 ON f1.id_persona = pf1.id_persona
            LEFT JOIN usuarios u2 ON a.modificado_por = u2.id_usuario
            LEFT JOIN funcionarios f2 ON u2.id_funcionario = f2.id_funcionario
            LEFT JOIN personas pf2 ON f2.id_persona = pf2.id_persona
        ...
        try:
            ...
            if not a:
                app.logger.info(f"No existe anamnesis activa para paciente ID: {pac_id}")
                return None
            ...
            return {
                ...
                'version': a[42],
                'activo': a[43],
                'nombre_paciente': a[44],
                'historia_clinica': a[45],
                'elaborado_por_nombre': a[46],
                'modificado_por_nombre': a[47]
            }
    ...
    def updateAnamnesis(self, anamnesis_id, usuario_id, motivo_consulta=None,
        ...
        updateAnamnesisSQL = """
            UPDATE anamnesis SET
                ...
                modificado_por = %s,
                version = version + 1
            WHERE id_anamnesis = %s AND activo = TRUE
        """
        ...
        try:
            ...
            insertHistorialSQL = """
                INSERT INTO anamnesis_historial(
                    id_anamnesis, version, contenido_json, modificado_por
                ) VALUES (%s, %s, %s, %s)
            """
    ...
    def getAllAnamnesis(self):
        """Obtiene todas las anamnesis activas (para listados)"""
        anamnesisSQL = """
            SELECT
                a.id_anamnesis,
                a.id_paciente,
                ...
                a.fecha_ultima_modificacion,
                a.version,
                pf.per_nombre || ' ' || pf.per_apellido AS elaborado_por_nombre
            FROM anamnesis a
            JOIN pacientes pac ON a.id_paciente = pac.id_paciente
            JOIN personas p ON pac.id_persona = p.id_persona
